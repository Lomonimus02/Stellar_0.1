...
import { UserRoleEnum, ChatTypeEnum, AcademicPeriodTypeEnum, GradingSystemEnum, studentClasses as studentClassesTable, attendance, studentSubgroups, classAcademicPeriods, academicPeriodBoundaries } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Helper function to get academic period dates (adapted from frontend)
  // This should ideally be in a shared utility or a backend-specific utility
  function getAcademicPeriodDates(
    periodKey: string,
    academicYear: number, // This is the starting year of the academic year, e.g., 2023 for 2023-2024
    classSpecificBoundaries?: schema.AcademicPeriodBoundary[]
  ): { startDate: Date; endDate: Date; label: string } {
    if (classSpecificBoundaries && classSpecificBoundaries.length > 0) {
      const boundary = classSpecificBoundaries.find(b => b.periodKey === periodKey && b.academicYear === `${academicYear}-${academicYear + 1}`);
      if (boundary) {
      const endDateBoundary = new Date(boundary.endDate);
      endDateBoundary.setHours(23, 59, 59, 999); // Set to end of day
        return {
    ...
    // Fallback to default period dates
  ...
  let defaultLabel: string;
    ...
    switch (periodKey) {
      case 'quarter1':
      defaultStartDate = new Date(academicYear, 8, 1); // September 1st
      defaultEndDate = new Date(academicYear, 9, 31); // October 31st
      defaultLabel = `1 четверть`;
      break;
      case 'quarter2':
      defaultStartDate = new Date(academicYear, 10, 1); // November 1st
      defaultEndDate = new Date(academicYear, 11, 31); // December 31st
      defaultLabel = `2 четверть`;
      break;
      case 'quarter3':
      defaultStartDate = new Date(academicYear + 1, 0, 1); // January 1st
      defaultEndDate = new Date(academicYear + 1, 2, 31); // March 31st
      defaultLabel = `3 четверть`;
      break;
      case 'quarter4':
      defaultStartDate = new Date(academicYear + 1, 3, 1); // April 1st
      defaultEndDate = new Date(academicYear + 1, 4, 31); // May 31st (adjust if needed for June)
      defaultLabel = `4 четверть`;
      break;
      case 'semester1':
      defaultStartDate = new Date(academicYear, 8, 1);
      defaultEndDate = new Date(academicYear, 11, 31);
      defaultLabel = `1 полугодие`;
      break;
      case 'semester2':
      defaultStartDate = new Date(academicYear + 1, 0, 1);
      defaultEndDate = new Date(academicYear + 1, 5, 30); // June 30th
      defaultLabel = `2 полугодие`;
      break;
      case 'trimester1':
      defaultStartDate = new Date(academicYear, 8, 1);
      defaultEndDate = new Date(academicYear, 10, 30); // November 30th
      defaultLabel = `1 триместр`;
      break;
      case 'trimester2':
      defaultStartDate = new Date(academicYear, 11, 1); // December 1st
      defaultEndDate = new Date(academicYear + 1, 1, 28); // End of February
      defaultLabel = `2 триместр`;
      break;
      case 'trimester3':
      defaultStartDate = new Date(academicYear + 1, 2, 1); // March 1st
      defaultEndDate = new Date(academicYear + 1, 4, 31); // May 31st
      defaultLabel = `3 триместр`;
      break;
      case 'year':
      defaultStartDate = new Date(academicYear, 8, 1);
      defaultEndDate = new Date(academicYear + 1, 5, 30); // June 30th
      defaultEndDate.setHours(23, 59, 59, 999); // Устанавливаем конец дня для корректного сравнения
      defaultLabel = `Учебный год ${academicYear}-${academicYear + 1}`;
      break;
    default: 
        console.warn(`Unknown periodKey: ${periodKey}, defaulting to Quarter 1 for year ${academicYear}`);
      defaultStartDate = new Date(academicYear, 8, 1);
      defaultEndDate = new Date(academicYear, 9, 31);
      defaultLabel = `1 четверть (default)`;
      break;
  }
  defaultEndDate.setHours(23, 59, 59, 999); // Set to end of day for default dates too
  return {
  ...
  async function generateDocxBuffer(student: schema.User, grades: schema.Grade[], periodLabel: string, academicYearLabel: string): Promise<Buffer> {
    const studentFullName = `${student.lastName} ${student.firstName}`;
    const title = `Отчет об успеваемости ученика`;
    const subTitle = `${studentFullName} за ${periodLabel}, ${academicYearLabel} учебный год`;

    const headerTexts = ["Дата", "Предмет", "Тип оценки", "Оценка", "Комментарий", "Учитель"];
    const tableHeader = new TableRow({
      children: headerTexts.map(text => new TableCell({
        children: [new Paragraph({
          children: [new TextRun({ text, bold: true })],
          alignment: AlignmentType.CENTER,
        })],
        width: { size: text === "Комментарий" ? 25 : (text === "Предмет" ? 20 : 15), type: WidthType.PERCENTAGE },
      })),
    });
    ...
    if (grades.length === 0) {
      // Если оценок нет, добавляем одну строку с сообщением
      dataRows = [
        new TableRow({
          children: [
            new TableCell({
              children: [new Paragraph({
                text: "Нет оценок за выбранный период",
                alignment: AlignmentType.CENTER
              })],
              columnSpan: 6 // Объединяем все ячейки в строке
            })
          ]
        })
      ];
    } else {
      // Если оценки есть, обрабатываем их как обычно
      dataRows = await Promise.all(grades.map(async (grade) => {
        let subjectName = `Предмет ID: ${grade.subjectId}`;
        try {
          const subject = await dataStorage.getSubject(grade.subjectId);
          if (subject) subjectName = subject.name;
        } catch (e) { console.error(`Failed to fetch subject ${grade.subjectId}`, e); }

        let teacherName = `-`;
        if (grade.teacherId) {
            try {
                const teacher = await dataStorage.getUser(grade.teacherId);
                if (teacher) {
                    const decryptedTeacher = decryptUserFields(teacher); // Decrypt teacher names
                    teacherName = `${decryptedTeacher.lastName} ${decryptedTeacher.firstName.charAt(0)}.`;
                }
            } catch (e) { console.error(`Failed to fetch teacher ${grade.teacherId}`, e); }
        }
        
        return new TableRow({
          children: [
            new TableCell({ children: [new Paragraph(format(new Date(grade.createdAt), 'dd.MM.yyyy'))] }),
            new TableCell({ children: [new Paragraph(subjectName)] }),
            new TableCell({ children: [new Paragraph(grade.gradeType || '-')] }),
            new TableCell({ children: [new Paragraph({ text: String(grade.grade), alignment: AlignmentType.CENTER })] }),
            new TableCell({ children: [new Paragraph(grade.comment || '-')] }),
            new TableCell({ children: [new Paragraph(teacherName)] }),
          ],
        });
      }));
    }
    ...
    const table = new Table({
      ...
      columnWidths: [1500, 2500, 1500, 1000, 3000, 1500], // Example widths, adjust as needed
    });
  ...
  app.get("/api/classes", isAuthenticated, async (req, res) => {
    ...
    for (const classObj of classes) {
      ...
      (classObj as any).studentCount = studentsInClass.length;
      ...
      if (classTeacherRole) {
        ...
        if (teacher) {
          ...
          (classObj as any).classTeacher = null;
        }
      } else {
        (classObj as any).classTeacher = null;
      }
  ...
  app.get("/api/student-subject-averages", isAuthenticated, async (req, res) => {
    try {
      ...
      let allClassGrades = await dataStorage.getGradesByClass(parsedClassId);
      console.log(`getGradesByClass вернул ${allClassGrades?.length || 0} оценок`);
      
      // Если оценок нет через getGradesByClass, попробуем получить их через студентов класса
      if (allClassGrades.length === 0) {
        console.log(`Оценки через getGradesByClass не найдены, пробуем получить через студентов класса`);
        
        // Получаем всех студентов класса
        const classStudents = await dataStorage.getClassStudents(parsedClassId);
        console.log(`Получено ${classStudents.length} студентов класса ${parsedClassId}`);
        
        // Для каждого студента получаем его оценки
        const allStudentGrades = [];
        for (const student of classStudents) {
          if (student && student.id) {
            const studentGrades = await dataStorage.getGradesByStudent(student.id);
            console.log(`Получено ${studentGrades.length} оценок для студента ${student.id}`);
            allStudentGrades.push(...studentGrades);
          }
        }
        
        // Фильтруем оценки, чтобы исключить дубликаты
        const uniqueGradeIds = new Set();
        allClassGrades = allStudentGrades.filter(grade => {
          if (!grade || !grade.id) return false;
          if (uniqueGradeIds.has(grade.id)) return false;
          uniqueGradeIds.add(grade.id);
          return true;
        });
        
        console.log(`Всего получено ${allClassGrades.length} уникальных оценок для класса ${parsedClassId} через студентов`);
      }
      
      // Фильтруем оценки по дате, если указан период
      const filteredGrades = startDate && endDate
        ? allClassGrades.filter(grade => {
            if (!grade || !grade.createdAt) return false; // Пропускаем оценки без даты
            
            try {
              const gradeDate = new Date(grade.createdAt);
              const isInRange = gradeDate >= startDate && gradeDate <= endDate;
              
              if (isInRange) {
                console.log(`Оценка ${grade.id} (студент ${grade.studentId}, предмет ${grade.subjectId}) попадает в диапазон дат`);
              }
      ...
      console.log(`После фильтрации по дате осталось ${filteredGrades.length} оценок из ${allClassGrades.length}`);
      
      // Группируем оценки по студентам для удобства
      const gradesByStudent = {};
      for (const grade of filteredGrades) {
        if (!grade || !grade.studentId) continue;
      ...
      console.log(`Оценки сгруппированы по ${Object.keys(gradesByStudent).length} студентам`);
      ...
      for (const student of students) {
        ...
        // Инициализируем объект для студента с пустыми значениями для overall
        result[studentId] = {
          overall: { average: "-", percentage: "-" }
        };
        
        // Получаем оценки для этого ученика из сгруппированного объекта
        const studentGrades = gradesByStudent[studentId] || [];
        console.log(`Для студента ${studentId} найдено ${studentGrades.length} оценок`);
        ...
        // Если у ученика нет оценок, оставляем значения по умолчанию
        if (studentGrades.length === 0) {
          console.log(`У ученика ${student.firstName} ${student.lastName} (ID: ${studentId}) нет оценок за указанный период`);
        ...
        for (const subjectId of studentSubjectIds) {
          ...
          if (subjectGrades.length > 0) {
            ...
            if (classInfo.gradingSystem === GradingSystemEnum.CUMULATIVE || classInfo.gradingSystem === 'cumulative') {
        ...
        if (studentGrades.length > 0) {
          ...
          // Оставляем значения по умолчанию, которые уже установлены при инициализации
          console.log(`У ученика ${student.firstName} ${student.lastName} (ID: ${studentId}) нет оценок за указанный период`);
        }
      }
      ...
      if (req.query.studentId) {
        ...
        if (req.user.role === UserRoleEnum.STUDENT) {
          ...
          const decryptedAttendance = filteredAttendance.map(att => {
              if ((att as any).student) { // Assuming student object is embedded
                  return { ...att, student: decryptUserFields((att as any).student) };
              }
              return att;
          });
  ...
  app.get("/api/messages", isAuthenticated, async (req, res) => {
    ...
    // Decrypt sender/receiver info if embedded
     const decryptedMessages = messages.map(msg => {
         ...
         if ((msg as any).sender) {
             (decryptedMsg as any).sender = decryptUserFields((msg as any).sender);
         }
         if ((msg as any).receiver) {
             (decryptedMsg as any).receiver = decryptUserFields((msg as any).receiver);
         }
         return decryptedMsg;
     });
  ...
  app.post("/api/messages", isAuthenticated, async (req, res) => {
    ...
    // Note: receiverId is not available in the message object,
    // notifications should be handled differently for chat messages
  ...
  app.post("/api/messages/:id/read", isAuthenticated, async (req, res) => {
    ...
    // Ensure the user is not the sender (can only mark received messages as read)
    if (message.senderId === req.user!.id) {
      return res.status(403).json({ message: "You cannot mark your own messages as read" });
    }
  ...
  app.post("/api/teacher-subjects", hasRole([UserRoleEnum.SUPER_ADMIN, UserRoleEnum.SCHOOL_ADMIN]), async (req, res) => {
    ...
    const teacherRoles = await dataStorage.getUserRoles(teacherId);
    const hasTeacherRole = teacherRoles.some(role => role.role === UserRoleEnum.TEACHER);
    if (!hasTeacherRole) {
  ...
  app.get("/api/my-roles", isAuthenticated, async (req, res) => {
    ...
    if (activeRoleExists) {
      ...
      for (const role of result) {
        (role as any).isActive = role.role === req.user!.activeRole;
      }
    } else {
      // Если активной роли нет или она была удалена, пометим первую роль как активную
      if (result.length > 0) {
        (result[0] as any).isActive = true;
  ...
  app.get("/api/student-subjects/:studentId", isAuthenticated, async (req, res) => {
    try {
      ...
      const studentClass = await dataStorage.getStudentCurrentClass(studentId);
      if (!studentClass) { // getStudentCurrentClass возвращает один класс или null
      ...
      if (!studentClass.id || isNaN(parseInt(String(studentClass.id)))) {
        console.error(`Invalid classId (${studentClass.id}) for student ${studentId} in student-subjects route.`);
        return res.status(500).json({ message: "Internal server error: Invalid class data for student." });
      }
      const schedules = await dataStorage.getSchedulesByClass(studentClass.id);
      ...
      for (const subjectId of subjectIds) {
        if (subjectId === null || subjectId === undefined) {
          console.warn(`Skipping fetch for null or undefined subjectId in student-subjects route for student ${studentId}`);
          continue;
        }
        const subject = await dataStorage.getSubject(subjectId);
        if (subject) {
  ...
  app.post("/api/chats", isAuthenticated, async (req, res) => {
    try {
      ...
      // Для личных чатов проверяем, что не существует уже чат между этими пользователями
      if (type === ChatTypeEnum.PRIVATE) {
        ...
        const existingChat = await dataStorage.findPrivateChatBetweenUsers(currentUserId, otherUserId);
        if (existingChat) {
          return res.status(409).json({
            message: "Private chat between these users already exists",
  ...
  // Добавление участника в групповой чат (только для создателя)
  app.post("/api/chats/:chatId/participants", isAuthenticated, async (req, res) => {
    try {
      const chatId = parseInt(req.params.chatId);
      ...
      if (chat.creatorId !== req.user.id) {
        return res.status(403).json({ message: "Only chat creator can add participants" });
      }

      // Проверяем, существует ли пользователь для добавления
      const userToAdd = await dataStorage.getUser(userIdToAdd);
      if (!userToAdd) {
        return res.status(404).json({ message: "User not found" });
      }
      ...
      const participants = await dataStorage.getChatParticipants(chatId);
      const isAlreadyParticipant = participants.some(p => p.userId === userIdToAdd);
  ...
  // Удаление участника из группового чата (только для создателя)
  app.delete("/api/chats/:chatId/participants/:userId", isAuthenticated, async (req, res) => {
    try {
      const chatId = parseInt(req.params.chatId);
      ...
      // Проверяем, является ли текущий пользователь создателем чата
      if (chat.creatorId !== req.user.id) {
        return res.status(403).json({ message: "Only chat creator can remove participants" });
      }

      // Проверяем, что пользователь не пытается удалить самого себя
      if (userIdToRemove === req.user.id) {
        return res.status(400).json({ message: "Cannot remove yourself from chat. Use leave endpoint instead" });
      }

      // Проверяем, является ли удаляемый пользователь участником чата
      const participants = await dataStorage.getChatParticipants(chatId);
      const participantToRemove = participants.find(p => p.userId === userIdToRemove);
      ...
      await dataStorage.removeChatParticipant(chatId, userIdToRemove);
  ...
  app.get("/api/chats/:chatId/messages", isAuthenticated, async (req, res) => {
    try {
      ...
      // Получаем сообщения с информацией об ответах
      const messages = await dataStorage.getChatMessagesWithReplies(chatId);
      ...
      const enrichedMessages = await Promise.all(
        messages.map(async (message) => {
          ...
          // Если есть ответ на сообщение, получаем информацию об отправителе исходного сообщения
          let replyToMessage = undefined;
          if (message.replyToMessage) {
            const replyToSender = await dataStorage.getUser(message.replyToMessage.senderId);
            replyToMessage = {
  ...
  app.post("/api/chats/:chatId/avatar", isAuthenticated, upload.single('avatar'), async (req, res) => {
    try {
      ...
      // Проверяем, является ли пользователь создателем группового чата
      if (chat.type === 'group' && chat.creatorId !== req.user.id) {
        return res.status(403).json({ message: "Only chat creator can change group avatar" });
      }

      // Читаем файл и конвертируем в Base64
      const imageData = fsSync.readFileSync(req.file.path, { encoding: 'base64' });
      ...
      await dataStorage.deleteChatAvatar(chatId);

      // Создаем новую аватарку
      const avatar = await dataStorage.createChatAvatar(avatarData);

      // Обновляем флаг наличия аватарки в чате
      await dataStorage.updateChat(chatId, { hasAvatar: true });
  ...
  // API для получения аватарки чата
  app.get("/api/chats/:chatId/avatar", async (req, res) => {
    try {
      const chatId = parseInt(req.params.chatId);

      const avatar = await dataStorage.getChatAvatar(chatId);
      if (!avatar) {
      ...
      const etag = `"${avatar.id}-${new Date(avatar.updatedAt).getTime()}"`;
      res.set({
        'Content-Type': avatar.mimeType,
        'Content-Length': imageBuffer.length,
        'Cache-Control': 'public, max-age=31536000', // Кэшируем на год
        'ETag': etag // ETag для кэширования
      });

      res.send(imageBuffer);
    } catch (error) {
      console.error("Error serving chat avatar:", error);
      res.status(500).json({ message: "Failed to serve avatar" });
    }
  });

  // API для удаления аватарки чата
  app.delete("/api/chats/:chatId/avatar", isAuthenticated, async (req, res) => {
    try {
      const chatId = parseInt(req.params.chatId);
      ...
      // Проверяем, является ли пользователь создателем группового чата
      if (chat.type === 'group' && chat.creatorId !== req.user.id) {
        return res.status(403).json({ message: "Only chat creator can delete group avatar" });
      }
      ...
      // Обновляем флаг наличия аватарки в чате
      await dataStorage.updateChat(chatId, { hasAvatar: false });
  ...
  app.post("/api/chats/:chatId/messages", isAuthenticated, async (req, res) => {
    try {
      const chatId = parseInt(req.params.chatId);
      const { content, hasAttachment, attachmentType, attachmentUrl, replyToMessageId } = req.body;
      ...
      // Если это ответ на сообщение, проверяем что исходное сообщение существует и принадлежит этому чату
      if (replyToMessageId) {
        const replyToMessage = await dataStorage.getMessage(replyToMessageId);
        if (!replyToMessage || replyToMessage.chatId !== chatId) {
          return res.status(400).json({ message: "Invalid reply to message ID" });
        }
      }
      ...
      // Если это ответ на сообщение, получаем информацию об исходном сообщении
      let replyToMessage = undefined;
      if (newMessage.replyToMessageId) {
        const originalMessage = await dataStorage.getMessage(newMessage.replyToMessageId);
        if (originalMessage) {
          const originalSender = await dataStorage.getUser(originalMessage.senderId);
          replyToMessage = {
            ...originalMessage,
            sender: {
              id: originalSender.id,
              firstName: originalSender.firstName,
              lastName: originalSender.lastName
            }
          };
        }
      }
  ...
  app.get("/api/academic-periods/:classId", isAuthenticated, hasRole([UserRoleEnum.SCHOOL_ADMIN, UserRoleEnum.PRINCIPAL, UserRoleEnum.VICE_PRINCIPAL, UserRoleEnum.TEACHER, UserRoleEnum.CLASS_TEACHER]), async (req, res) => {
    try {
      const classId = parseInt(req.params.classId);
      if (isNaN(classId)) {
        return res.status(400).json({ message: "Invalid class ID" });
  ...
  // DOCX Export Route
  app.get("/api/export/student-grades/:studentId/docx", isAuthenticated, async (req, res) => {
    try {
      const studentId = parseInt(req.params.studentId);
      const periodKey = req.query.periodKey as string;
      const academicYearParam = req.query.academicYear as string;

      if (isNaN(studentId) || !periodKey || !academicYearParam || isNaN(parseInt(academicYearParam))) {
        return res.status(400).json({ message: "Invalid parameters: studentId, periodKey, and academicYear are required." });
      }
      const academicYear = parseInt(academicYearParam);

      // --- Permission Check (similar to student-details page) ---
      const currentUser = req.user;
      const studentUser = await dataStorage.getUser(studentId);
      ...
      let hasAccess = false;
      if (userHasRole(currentUser, schema.UserRoleEnum.SUPER_ADMIN)) {
        hasAccess = true;
      } else if (userHasRole(currentUser, schema.UserRoleEnum.SCHOOL_ADMIN) ||
                 userHasRole(currentUser, schema.UserRoleEnum.PRINCIPAL) ||
                 userHasRole(currentUser, schema.UserRoleEnum.VICE_PRINCIPAL)) {
        if (currentUser.schoolId === studentUser.schoolId) {
          hasAccess = true;
        }
      } else if (userHasRole(currentUser, schema.UserRoleEnum.CLASS_TEACHER)) {
        const ctRoles = await dataStorage.getUserRoles(currentUser.id);
        const studentClass = await dataStorage.getStudentCurrentClass(studentId);
        if (studentClass && ctRoles.some(r => r.role === schema.UserRoleEnum.CLASS_TEACHER && r.classId === studentClass.id)) {
          hasAccess = true;
        }
      } else if (userHasRole(currentUser, schema.UserRoleEnum.TEACHER)) {
        // Teachers can view students in classes they teach. This might be complex.
        // For simplicity, we'll allow if the teacher has taught the student in any subject.
        // A more robust check would involve checking schedules for the student's class and teacher.
        const studentSchedules = await dataStorage.getSchedulesByStudent(studentId); // Assuming this function exists or can be made
        if (studentSchedules.some(s => s.teacherId === currentUser.id)) {
            hasAccess = true;
        } else {
          // Fallback: Check if teacher teaches any subject in the student's class
          const studentClass = await dataStorage.getStudentCurrentClass(studentId);
          if (studentClass) {
              const classSchedules = await dataStorage.getSchedulesByClass(studentClass.id);
              if (classSchedules.some(s => s.teacherId === currentUser.id)) {
                  hasAccess = true;
              }
          }
        }
      } else if (userHasRole(currentUser, schema.UserRoleEnum.STUDENT) && currentUser.id === studentId) {
        hasAccess = true; // Student can export their own grades
      } else if (userHasRole(currentUser, schema.UserRoleEnum.PARENT)) {
        const parentStudents = await dataStorage.getParentStudents(currentUser.id);
        if (parentStudents.some(ps => ps.studentId === studentId)) {
          hasAccess = true; // Parent can export their child's grades
        }
      }

      if (!hasAccess) {
        return res.status(403).json({ message: "Forbidden. You do not have access to this student's grades." });
      }
      // --- End Permission Check ---

      const studentClass = await dataStorage.getStudentCurrentClass(studentId);
      let classSpecificBoundaries: schema.AcademicPeriodBoundary[] = [];
      if (studentClass) {
          const boundariesData = await db
              .select()
              .from(schema.academicPeriodBoundaries)
              .where(eq(schema.academicPeriodBoundaries.classId, studentClass.id));
          classSpecificBoundaries = boundariesData;
      }


      const { startDate, endDate, label: periodLabel } = getAcademicPeriodDates(periodKey, academicYear, classSpecificBoundaries);

      // Fetch grades for the student within the date range
      const allStudentGrades = await dataStorage.getGradesByStudent(studentId);

    console.log(`[Export DOCX] Student ID: ${studentId}, Period Key: ${periodKey}, Academic Year: ${academicYear}`);
    console.log(`[Export DOCX] Calculated Period: ${periodLabel}, Start Date: ${startDate.toISOString()}, End Date: ${endDate.toISOString()}`);
    if (allStudentGrades.length > 0) {
      console.log(`[Export DOCX] First 3 student grades (createdAt):`);
      allStudentGrades.slice(0, 3).forEach(g => console.log(` - ${new Date(g.createdAt).toISOString()} (Grade: ${g.grade})`));
    } else {
      console.log(`[Export DOCX] No grades found for student ${studentId} at all.`);
    }
    
      const gradesForPeriod = allStudentGrades.filter(grade => {
        const gradeDate = new Date(grade.createdAt);
        // Устанавливаем время для корректного сравнения
        gradeDate.setHours(12, 0, 0, 0); // Устанавливаем полдень для избежания проблем с часовыми поясами
        
        const isInPeriod = gradeDate >= startDate && gradeDate <= endDate;
        console.log(`[Export DOCX] Grade ID ${grade.id} (${gradeDate.toISOString()}) in period? ${isInPeriod}. Start: ${startDate.toISOString()}, End: ${endDate.toISOString()}`);
        return isInPeriod;
      });

    console.log(`[Export DOCX] Total grades for student: ${allStudentGrades.length}, Grades in period ${periodLabel}: ${gradesForPeriod.length}`);

      // Даже если нет оценок за период, мы все равно создаем документ
      // Это позволит избежать ошибки 404, когда оценки отображаются на странице, но не попадают в экспорт
      if (gradesForPeriod.length === 0) {
        console.log(`[Export DOCX] No grades found for period, but continuing with empty grades list`);
        // Продолжаем выполнение и создаем пустой документ вместо возврата ошибки
      }

      const decryptedStudent = decryptUserFields(studentUser);
      const studentFullName = `${decryptedStudent.lastName} ${decryptedStudent.firstName}`;
      const fileName = `grades_${studentUser.username}_${periodKey}_${academicYear}.docx`;


      res.setHeader('Content-Disposition', `attachment; filename="${fileName}"`);
      res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document');
      
      const docBuffer = await generateDocxBuffer(decryptedStudent, gradesForPeriod, periodLabel, `${academicYear}-${academicYear + 1}`);
      res.send(docBuffer);

    } catch (error) {
      console.error("Error exporting student grades to DOCX:", error);
      res.status(500).json({ message: "Failed to export grades to DOCX.", error: error.message });
    }
  });
