...
import ClassesManagementPage from "./pages/classes-management";
...
function Router() {
  return (
    <Switch>
      ...
      <ProtectedRoute path="/classes-management/:classId" component={ClassDetailsPage} />
      <ProtectedRoute path="/classes-management" component={ClassesManagementPage} />
      <ProtectedRoute path="/student-details/:studentId" component={StudentDetailsPage} />
      ...
      <ProtectedRoute path="/academic-periods" component={AcademicPeriodsPage} />
...
function App() {
  // Обработчик для улучшения SPA навигации
  useEffect(() => {
    const handlePopState = (event: PopStateEvent) => {
      // Предотвращаем перезагрузку страницы при навигации назад/вперед
      // wouter автоматически обрабатывает изменения URL
      console.log('SPA Navigation: popstate event handled', event.state);
    };
