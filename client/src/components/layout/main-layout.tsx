...
export function MainLayout({ children, className }: MainLayoutProps) {
  ...
  useEffect(() => {
    const handleContextMenu = (event: MouseEvent) => {
      ...
      // Check if the right-click is inside a chat context menu area
      const target = event.target as Element;

      // Look for chat context menu triggers or content
      const isInsideChatContextMenu = target.closest('[data-radix-context-menu-trigger]') ||
                                     target.closest('[data-radix-context-menu-content]') ||
                                     target.closest('[data-chat-item]') ||
                                     target.closest('[data-chat-list]') ||
                                     // Check for messages page specifically
                                     (window.location.pathname === '/messages' &&
                                      (target.closest('.md\\:col-span-1') ||
                                       target.closest('.md\\:col-span-2')));

      console.log('[MainLayout] Right-click detected:', {
        pathname: window.location.pathname,
        targetElement: target.tagName,
        targetClasses: target.className,
        isInsideChatContextMenu,
        hasDataChatItem: !!target.closest('[data-chat-item]'),
        hasDataChatList: !!target.closest('[data-chat-list]'),
        hasRadixContextMenu: !!target.closest('[data-radix-context-menu-trigger]'),
        isMessagesPage: window.location.pathname === '/messages'
      });

      if (isInsideChatContextMenu) {
        console.log('[MainLayout] Right-click inside chat area - allowing default behavior');
        // Allow the chat context menu to handle the right-click
        return;
      }

      // From this point, RMB control is ON, and it IS a right-click outside chat areas
