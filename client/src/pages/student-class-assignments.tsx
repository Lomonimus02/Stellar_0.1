...
import { UserPlusIcon, BookIcon, CheckIcon, XIcon, SearchIcon, Trash2Icon } from "lucide-react";
...
export default function StudentClassAssignmentsPage() {
  ...
  const addStudentToClassMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        title: "Успешно",
        description: "Ученик переведен в класс",
        variant: "default",
      });
      // Сбрасываем форму
      form.reset();
    ...
    onError: (error) => {
      toast({
        title: "Ошибка",
        description: error.message || "Не удалось перевести ученика в класс",
        variant: "destructive",
      });
    },
  });

  // Мутация для удаления ученика из класса
  const removeStudentFromClassMutation = useMutation({
    mutationFn: async (data: { studentId: number, classId: number }) => {
      const res = await apiRequest("DELETE", `/api/student-classes?studentId=${data.studentId}&classId=${data.classId}`);
      return res.json();
    },
    onSuccess: () => {
      toast({
        title: "Успешно",
        description: "Ученик удален из класса",
        variant: "default",
      });
      // Обновляем данные
      if (selectedStudent) {
    ...
    onError: (error) => {
      toast({
        title: "Ошибка",
        description: error.message || "Не удалось удалить ученика из класса",
        variant: "destructive",
      });
    },
  });
  ...
  // Обработчик удаления ученика из класса
  const handleRemoveFromClass = (studentId: number, classId: number) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card className="md:col-span-2">
            <CardHeader>
              ...
              <CardDescription>
                {selectedStudent ? "Ученик может состоять только в одном классе одновременно" : "Для управления классами сначала выберите ученика из списка слева"}
              </CardDescription>
            </CardHeader>
            <CardContent>
              {selectedStudent && (
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 mb-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>Новый класс</FormLabel>
                              <Select
                                onValueChange={field.onChange}
                              ...
                              >
                                ...
                                <SelectContent>
                                  {classes.map((cls) => (
                                    ...
                                    >
                                      {cls.name} {isStudentInClass(selectedStudent, cls.id) && "(текущий класс)"}
                                    </SelectItem>
                                  ))}
                                </SelectContent>
                              </Select>
                      ...
                      <Button type="submit" disabled={addStudentToClassMutation.isPending}>
                        {addStudentToClassMutation.isPending ? "Перевод..." : "Перевести в класс"}
                      </Button>
                    </form>
                  </Form>

                  <div className="mt-6">
                    <h3 className="text-lg font-medium mb-4">Текущий класс ученика</h3>
                    {studentClassesLoading ? (
                      <div className="text-center p-4">Загрузка класса...</div>
                            ...
                            <TableRow key={cls.id}>
                              ...
                              <TableCell>
                                <Button
                                  variant="destructive"
                                  size="sm"
                                  onClick={() => handleRemoveFromClass(selectedStudent!, cls.id)}
