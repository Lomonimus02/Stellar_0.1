...
import { Subject, Subgroup, Class, InsertSubgroup, InsertSubject, School, Schedule, User, UserRoleEnum } from "@shared/schema";
...
export default function SubjectsManagementPage() {
  ...
  const { data: teachers = [], isLoading: teachersLoading } = useQuery<User[]>({
    queryKey: ["/api/users", { role: UserRoleEnum.TEACHER, schoolId }],
    queryFn: async () => {
      if (!schoolId) return [];
      const res = await apiRequest(`/api/users?role=${UserRoleEnum.TEACHER}&schoolId=${schoolId}`);
      return res.json();
    },
    enabled: !!schoolId && isAdmin(),
  });
  ...
  const isLoading = subjectsLoading || subgroupsLoading || classesLoading || schedulesLoading || teachersLoading;

  // --- Helper Functions ---
  const getSubjectTeacherForClass = useCallback((classId: number, subjectId: number) => {
    // Находим расписание для данного класса и предмета
    const schedule = schedules.find(s => s.classId === classId && s.subjectId === subjectId);
    if (!schedule) return null;

    // Находим учителя по ID из расписания
                ...
                {isLoadingRelatedClasses ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                    {relatedClasses.map((cls) => {
                      // Определяем ID предмета для поиска учителя
                      const isSubgroup = 'isSubgroup' in selectedItem;
                      let subjectIdForTeacher = isSubgroup ? null : selectedItem.id;

                      // Если это подгруппа, находим предмет через расписание
                      if (isSubgroup) {
                        const scheduleWithSubgroup = schedules.find(s => s.subgroupId === selectedItem.id && s.classId === cls.id);
                        subjectIdForTeacher = scheduleWithSubgroup?.subjectId || null;
                      }

                      // Получаем учителя предмета для этого класса
                      const subjectTeacher = subjectIdForTeacher ? getSubjectTeacherForClass(cls.id, subjectIdForTeacher) : null;
