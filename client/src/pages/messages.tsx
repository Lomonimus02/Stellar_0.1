...
export default function MessagesPage() {
  ...
  const sendMessageMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setReplyToMessage(null); // Очищаем состояние ответа
    },
    onError: (error: any) => {
      toast({
        title: "Ошибка",
  ...
  const createChatMutation = useMutation({
    mutationFn: async (data: NewChatFormValues) => {
      try {
        ...
        // Если это ошибка 409 (конфликт), пытаемся извлечь детали
        if (error.message && error.message.includes('409:')) {
          try {
            ...
            // Если не удалось распарсить JSON, выбрасываем оригинальную ошибку
            throw error;
          }
        }
        throw error;
    ...
    onError: (error: any) => {
      // Обрабатываем случай, когда личный чат уже существует
      if (error.status === 409 && error.existingChatId) {
        ...
        toast({
          ...
          description: "Переключаемся на существующий личный чат",
        });
        return;
      }
  ...
  // Обработка создания чата через новый диалог
  ...
  // Состояние для управления диалогом подтверждения удаления участника
  ...
  const updateChatMutation = useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      // Обновляем selectedChat если это тот же чат
  ...
  // Мутация для добавления участника в чат
  ...
  // Мутация для удаления участника из чата
    ...
    <MainLayout>
      <div className="container mx-auto py-8 px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-3xl font-bold text-slate-700 mb-0">Сообщения</h2>
          <DropdownMenu>
                      ...
                      >
                        ...
                        >
                          <Avatar className="h-10 w-10 mr-3">
                            {console.log('🖼️ Rendering avatar for chat', chat.id, 'hasAvatar:', chat.hasAvatar, 'type:', chat.type)}
                            <AvatarImage
                              src={chat.hasAvatar ? `/api/chats/${chat.id}/avatar?t=${new Date(chat.updatedAt || Date.now()).getTime()}` : undefined}
                              alt={chatName}
                              onError={(e) => {
                                console.error('❌ Failed to load avatar for chat', chat.id, 'Error:', e);
                              }}
                              onLoad={() => {
                                console.log('✅ Successfully loaded avatar for chat', chat.id);
                              }}
                            />
                            <AvatarFallback className={isSelected ? 'bg-primary text-white' : 'bg-gray-200'}>
                              {console.log('🔄 Rendering fallback for chat', chat.id, 'type:', chat.type, 'hasAvatar:', chat.hasAvatar)}
                              {chat.type === ChatTypeEnum.GROUP ? (
                      ...
                      <div className="space-y-4 py-4">
                        {sortedMessages.map(message => {
                          ...
                          return (
                            ...
                            >
                              <div
                                className={`max-w-[80%] p-3 rounded-lg relative group cursor-pointer hover:shadow-md transition-shadow ${
                                  isSentByUser
                                    ? 'bg-primary text-white rounded-tr-none'
                                    : 'bg-gray-100 text-gray-800 rounded-tl-none'
                                }`}
                                onClick={() => setReplyToMessage(message)}
                                title="Нажмите, чтобы ответить на это сообщение"
                              >
                                ...
                                {message.replyToMessage && (
                                  <div className={`mb-2 p-2 rounded border-l-2 text-xs ${
                                    isSentByUser
                                      ? 'bg-primary-dark border-white/30 text-white/80'
                                    ...
                                    <p className="truncate">
                                      {message.replyToMessage.content || 'Вложение'}
                                    </p>
                                  </div>
                                )}
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <p className="text-xs text-gray-500 mb-1">
                            Ответ на сообщение от {replyToMessage.sender ?
                              `${replyToMessage.sender.firstName} ${replyToMessage.sender.lastName}` :
                              'Пользователь'
                            }
                          </p>
                          <p className="text-sm text-gray-700 truncate">
                            {replyToMessage.content || 'Вложение'}
                          </p>
                        </div>
                        <button
                          type="button"
      ...
      {chatToEdit && (
        <EditChatDialog
          ...
          onToggleAdmin={(userId) => {
            // TODO: Реализовать переключение статуса админа
      ...
      <AlertDialog open={removeParticipantAlertOpen} onOpenChange={setRemoveParticipantAlertOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Вы уверены, что хотите удалить {participantToRemove?.userName} из группы?
              Участник больше не сможет видеть сообщения и участвовать в беседе.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Отмена</AlertDialogCancel>
      ...
      {/* Новый диалог выбора пользователей */}
