...
import { getCurrentAcademicPeriod, type QuarterType } from "@/utils/academic-period";
...
export default function StudentGrades() {
  ...
  const [displayPeriod, setDisplayPeriod] = useState<QuarterType>(currentPeriodInfo.period);
  ...
  const { data: assignments = [] } = useQuery<Assignment[]>({
    ...
    enabled: !!user && isStudent() && studentClass && studentClass.length > 0 && grades.length > 0
  });
  ...
  const hasBlockingErrors = !user || !isStudent() || !!loadingErrors.auth;
