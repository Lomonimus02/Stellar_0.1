...
async function createAcademicPeriodsTables() {
  try {
    console.log('Проверка и создание таблиц для учебных периодов...');

    // Проверяем существование таблицы class_academic_periods
    const checkClassAcademicPeriodsTableQuery = `
      SELECT EXISTS (
        SELECT FROM information_schema.tables
        WHERE table_name = 'class_academic_periods'
      );
    `;
    const classAcademicPeriodsTableExists = (await db.execute(sql.raw(checkClassAcademicPeriodsTableQuery)))[0].exists;

    if (!classAcademicPeriodsTableExists) {
      console.log('Создание таблицы class_academic_periods...');
      await db.execute(sql.raw(`
      ...
      `));
      console.log('Таблица class_academic_periods успешно создана');
    } else {
      console.log('Таблица class_academic_periods уже существует');
    }

    // Проверяем существование таблицы academic_period_boundaries
    const checkAcademicPeriodBoundariesTableQuery = `
      SELECT EXISTS (
        SELECT FROM information_schema.tables
        WHERE table_name = 'academic_period_boundaries'
      );
    `;
    const academicPeriodBoundariesTableExists = (await db.execute(sql.raw(checkAcademicPeriodBoundariesTableQuery)))[0].exists;

    if (!academicPeriodBoundariesTableExists) {
      console.log('Создание таблицы academic_period_boundaries...');
      await db.execute(sql.raw(`
      ...
      `));
      console.log('Таблица academic_period_boundaries успешно создана');
    } else {
      console.log('Таблица academic_period_boundaries уже существует');
    }

    console.log('Все таблицы для учебных периодов готовы');
  } catch (error) {
    console.error('Ошибка при создании таблиц учебных периодов:', error);
    throw error;
  }
}
...
async function runMigrations() {
  try {
    ...
    await createAcademicPeriodsTables();
