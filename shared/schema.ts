...
// Enum для типов учебных периодов
export enum AcademicPeriodTypeEnum {
  QUARTERS = "quarters",     // Четверти
  SEMESTERS = "semesters",   // Полугодия
  TRIMESTERS = "trimesters"  // Триместры
}
...
export const studentClasses = pgTable("student_classes", {
  id: serial("id").primaryKey(),
  studentId: integer("student_id").notNull().unique(), // Студент может быть только в одном классе
...
// Чаты
// Новая таблица для хранения аватарок чатов
export const chatAvatars = pgTable("chat_avatars", {
  id: serial("id").primaryKey(),
  chatId: integer("chat_id").notNull(),                  // ID чата
  ...
  imageData: text("image_data").notNull(),               // Base64 encoded данные изображения
  createdAt: timestamp("created_at").defaultNow().notNull(),
...
export const chats = pgTable("chats", {
  ...
  hasAvatar: boolean("has_avatar").default(false),       // Флаг наличия аватарки
  createdAt: timestamp("created_at").defaultNow().notNull(),
...
export const messages = pgTable("messages", {
  ...
  replyToMessageId: integer("reply_to_message_id"),      // ID сообщения, на которое отвечаем (NULL если не ответ)
});
...
// Таблица для хранения настроек учебных периодов классов
export const classAcademicPeriods = pgTable("class_academic_periods", {
  id: serial("id").primaryKey(),
  classId: integer("class_id").notNull(), // Класс, для которого настроены периоды
  periodType: text("period_type").$type<AcademicPeriodTypeEnum>().notNull(), // Тип периода (четверти/полугодия/триместры)
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});

// Таблица для хранения границ конкретных учебных периодов
export const academicPeriodBoundaries = pgTable("academic_period_boundaries", {
  id: serial("id").primaryKey(),
  classId: integer("class_id").notNull(), // Класс, для которого настроены границы
  periodKey: text("period_key").notNull(), // Ключ периода (quarter1, quarter2, semester1, trimester1, etc.)
  periodName: text("period_name").notNull(), // Название периода (1 четверть, 1 полугодие, etc.)
  startDate: date("start_date").notNull(), // Дата начала периода
  endDate: date("end_date").notNull(), // Дата окончания периода
  academicYear: text("academic_year").notNull(), // Учебный год (например, "2024-2025")
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
...
export const insertClassAcademicPeriodSchema = createInsertSchema(classAcademicPeriods).omit({
  id: true,
  createdAt: true,
  updatedAt: true
});

export const insertAcademicPeriodBoundarySchema = createInsertSchema(academicPeriodBoundaries).omit({
  id: true,
  createdAt: true,
  updatedAt: true
});
...
// Схемы и типы для аватарок чатов
export const insertChatAvatarSchema = createInsertSchema(chatAvatars);
export type InsertChatAvatar = z.infer<typeof insertChatAvatarSchema>;
export type ChatAvatar = typeof chatAvatars.$inferSelect;
...
export type InsertClassAcademicPeriod = z.infer<typeof insertClassAcademicPeriodSchema>;
export type ClassAcademicPeriod = typeof classAcademicPeriods.$inferSelect;

export type InsertAcademicPeriodBoundary = z.infer<typeof insertAcademicPeriodBoundarySchema>;
export type AcademicPeriodBoundary = typeof academicPeriodBoundaries.$inferSelect;
